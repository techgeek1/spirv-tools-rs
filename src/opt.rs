...
pub struct SpirvOptimizerOptions {
    raw: spv_optimizer_options
}

impl SpirvOptimizerOptions {
...
impl Drop for SpirvOptimizerOptions {
    fn drop(&mut self) {
        unsafe { spvOptimizerOptionsDestroy(self.raw); }
    }
}

impl Default for SpirvOptimizerOptions {
    fn default() -> Self {
        Self::new()
    }
}
...
impl SpirvOptimizer {
    ...
    ///      (SpirvOptimizer::RegisterSizePasses).
    ///
    ...
    pub fn run_with_options(&self, binary: &[u32], options: SpirvOptimizerOptions) -> Result<Vec<u32>, OptimizerError> {
        unsafe {
            let bin_ptr = binary.as_ptr();
            let bin_len = binary.len();
